...
int main() {
    ...
    for(int i=0; i<2; i++){
        ...
        for(int k=0; k<3; k++){
            ...
            std::string folder = "/Users/michau/Documents/MOFIT1/results_3/";
            ...
            if(k==0){  
                while(test.back().t <100){
                    Stan u2, u12;
                    u2 = Euler(test.back(), 2*dt, alpha);
                    u12 = Euler(Euler(test.back(), dt, alpha), dt, alpha);
        ...
        // }
                ...
                while(test.back().t <100){
                    Stan u2, u12;
                    u2 = RK4(test.back(), 2*dt, alpha);
                    u12 = RK4(RK4(test.back(), dt, alpha), dt, alpha);
                    double eps_x = abs((u2.x-u12.x)/(pow(2,d[k])-1));
                    double eps_v = abs((u2.v-u12.v)/(pow(2,d[k])-1));
                    double eps = (eps_x>eps_v ? eps_x : eps_v);
                    if(eps <= tol){
